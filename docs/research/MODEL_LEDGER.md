...
| **Last entry** | 2026-08-22 (Milestone 38: Hindi TTS selection — all 4 Kokoro hi voices measured (clean RT-WER 0.045-0.071, hm_psi best), upstream long-text SILENT TRUNCATION found (~510 phonemes), Supertonic hi hard-crashes on Devanagari numerals + >1189 chars, SPRINGLab F5-Hindi registered and measured, Piper hi voices BLOCKED on NC training data; recommendation: Kokoro Hindi (Option A) → M39) |
...
| Kokoro-82M | speech_synthesis | EN **hardened through the production path (M35): trap WER 0.0659, CER 0.0251, median wall 748 ms, ladder clean to c=8, Web E2E verified** · HI **M38: all 4 voices measured, clean RT-WER 0.045-0.071 / CER 0.018-0.026 (hm_psi best), RTF 0.17-0.19, zero probe failures; upstream path silently truncates >~350 chars (danda chunking = M39 correctness fix)** · AR none | Apache-2.0 (re-verified 2026-08-20) | **Approved for Adoption** — incumbent; **M38 recommends extending it to Hindi (Option A)**; production launch + HI voice pick founder-gated | 2026-08-22 |
...
| **Hindi TTS** | Which compliant path un-gates Hindi synthesis? | **Updated 2026-08-22 (M38): DECIDED at research level — Kokoro Hindi (Option A), M39 defined.** All 4 Kokoro hi voices measured clean RT-WER 0.045-0.071 (hm_psi best); upstream truncation + espeak thread-safety facts recorded; Supertonic demoted (Devanagari-digit + long-text crashes, OpenRAIL unchanged); SPRINGLab F5-Hindi (CC-BY-4.0, from-scratch) registered as the only new CLEAR specialist. Remaining gates: founder listening on the audition pack (voice pick) and the M39 go — the espeak exec-boundary policy was already closed at M35 ([M38 §21, §24-25](2026-08-22-hindi-tts-model-selection.md)). |
