...
| **Last entry** | 2026-08-25 (Milestone 45: Qwen3-TTS 0.6B runtime/streaming research — TRUE frame-level streaming built on the official runtime (TTFA 0.80 s, continuity exact) but no runtime reached RTF < 1 on the 8 GB laptop GPU (best 1.39; vLLM-Omni 1.36-3.86 with ~6.5 s TTFA) → decision C, path deferred; re-entry = rented-GPU rerun of the unchanged harnesses. Kokoro keeps production) |
