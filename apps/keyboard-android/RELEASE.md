...
## Production endpoint (Milestone 20 note)

The release APK ships with an EMPTY default server address on purpose:
a production domain is a deliberate configuration, never a hardcoded
guess. When the production domain exists, the one-line change reserved
in `app/build.gradle.kts` (release `DEFAULT_BASE_URL`) bakes it in;
until then, operators set the server address in the app's settings.
Release builds are HTTPS-only (no cleartext config is merged —
guard-tested), so an `http://` production URL cannot work by design.

## Known client limitation: long audio (documented, deliberately unfixed)

The backend supports audio up to **600 seconds** since Milestone 19
(chunked internally; the client sees one ordinary response). This
client's HTTP budgets predate that: **okhttp read timeout 120 s, call
timeout 150 s** ([IntelliAIApiClient.kt](app/src/main/java/com/intelliai/keyboard/api/IntelliAIApiClient.kt)),
sized when the gateway's own deadline was 120 s.

Measured server walls (M19): a 300 s file decodes in ~80–155 s —
MARGINAL against the 150 s call cap; a 600 s file decodes in
~180–340 s — IMPOSSIBLE under it. Keyboard dictation (seconds of
audio) is completely unaffected.

The change a long-audio release would need: raise `readTimeout`/
`callTimeout` above the gateway's 450 s deadline plus margin, and
decide the UX for a multi-minute wait inside an IME (a keyboard is the
wrong surface for ten-minute uploads — this is a product decision, not
a constant to bump). That decision belongs to a dedicated Android
long-audio milestone; nothing in this repository raises those budgets
silently.
