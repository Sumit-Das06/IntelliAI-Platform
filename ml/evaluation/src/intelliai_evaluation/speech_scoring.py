...
def score_case(
    ...
    return CaseResult(case_id=case.id, transcript=transcript, metrics=metrics)


def aggregate_cases(cases: list[CaseResult]) -> dict[str, float]:
    """Means over the cases that measured each metric (failed stages simply
    contributed fewer samples — never zeros)."""
    sums: dict[str, float] = {}
    counts: dict[str, int] = {}
    for case in cases:
