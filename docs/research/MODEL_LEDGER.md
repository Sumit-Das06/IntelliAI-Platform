...
| **Last entry** | 2026-08-27 (Milestone 52: realtime STT feasibility — decision B, feasible with architectural changes: whisper-small re-decode streaming works (CPU ~1.5 s cadence, GPU passes every proposed gate incl. 446 ms partials), Qwen E3 CPU is realtime-blocked (1 s audio → 1.3 s inference, no state reuse), isolated WS prototype proved the session/backpressure/auth contract; proposed next = M53 realtime English web implementation. No production change) |
