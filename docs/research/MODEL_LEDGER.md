...
| **Last entry** | 2026-08-27 (Milestone 52H: Hindi Qwen E3 GPU realtime feasibility — decision B, feasible with minor architectural work: unchanged E3 GGUF on the RTX 5070 via the SAME-commit CUDA build runs 15-20x faster than CPU (1 s -> 64 ms, RTF ~0.055), FPT 490-660 ms on real Hindi speech, quality parity (25/30 byte-identical), 10 min bounded sessions, VRAM 2118 MiB flat; GPU is the named Hindi realtime deployment requirement. NEW OPEN FINDING: production service-path Hindi instability on a 31.8 s real clip — its own investigation. No production change) |
