"""Milestone 16 Phase 12: local mixed-traffic canary simulation.
...
operational shape of the proposed 90/10 split running through ONE
multi-slot process — error rate per arm, latency per arm, and whether
mixed routing destabilizes either engine. Quality is not re-measured
here; the switching EvalRuns own that.

The 90/10 split is deterministic (seeded), the clip cycle is fixed, and
each request pins its artifact exactly the way registry resolution
would downstream — this simulates the ROUTING OUTCOME, not the registry.
"""
...
def main() -> int:
    ...
    payload = {
        "simulation": "16-local-canary-90-10",
        "NOT_LEDGER_EVIDENCE": (
