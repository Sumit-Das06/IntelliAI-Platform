...
| **Last entry** | 2026-08-25 (Milestone 44: Qwen3-TTS 0.6B-Base measured with a calm clone reference — first clean-text WER win over Kokoro (trap 0.0515 vs 0.0659) but no streaming + RTF>1 → verdict D, Kokoro keeps production; official 0.6B fine-tune on public LJSpeech FAILED with text-conditioning collapse → verdict F; dataset `qwen-en-public-train@v1` frozen) |
...
| Qwen3-TTS | speech_synthesis | **MEASURED (M44, 0.6B-Base + calm LJ clone ref)**: trap 0.0515 / LJ 0.0478 — **beats the incumbent on clean text**; OOV 0.1724 (loses) · GPU RTF 1.4-1.6, CPU 2.85 · **still no streaming API** · official 0.6B fine-tune FAILED (text-conditioning collapse, verdict F) · 10 langs, no Indic | Apache-2.0 end-to-end — weights + `qwen-tts` lib (2026-08-20) | Researching — **M44 verdict: D, Kokoro still wins on UX (streaming/TTFA)**; revisit trigger: vLLM/official streaming with TTFA ≤ ~1 s; fine-tune re-entry only via 1.7B | 2026-08-25 |
