...
| **[research/RESEARCH_FRAMEWORK.md](research/RESEARCH_FRAMEWORK.md)** | The research lab's governing process for its two asset classes (foundation models, datasets): status lifecycle, stage gates, licensing review, benchmarking rules, multilingual strategy, technology watch, data flywheel, adopt-vs-tune tree — with the append-only [MODEL_LEDGER.md](research/MODEL_LEDGER.md) as the status of record | Permanent process (IN FORCE, founder-approved 2026-08-04; §16 priorities are living) | every engine research thread; capability openings; STT v2 / TTS v2 programs; the future dataset and training milestones |
