...
| **Last entry** | 2026-08-24 (Milestone 39: Hindi TTS IMPLEMENTED local/staging — hindi-female=hf_alpha / hindi-male=hm_psi through the full existing architecture; clean RT-WER 0.0707/0.0547 via the real gateway (gate ≤0.08 PASS), truncation defect re-measured GONE, stream TTFA 0.5-1.6 s, English battery 23/23; production untouched — staging proposal with PENDING sentinel) |
...
| Kokoro-82M | speech_synthesis | EN **hardened (M35): trap WER 0.0659, Web E2E verified** · HI **IMPLEMENTED local/staging (M39): hindi-female/hindi-male live behind the staging profile — clean RT-WER 0.0707/0.0547 via the real gateway, truncation fixed (danda chunking), stream TTFA 0.5-1.6 s, artifact v2 (+hf_alpha, +hm_psi)** · AR none | Apache-2.0 (re-verified 2026-08-20); Hindi G2P = pinned espeak-ng binary at the M35 exec boundary | **Approved for Adoption** — incumbent, EN+HI; production TTS launch + Hindi production promotion founder-gated (PENDING sentinel on the staging route) | 2026-08-24 |
...
| **Hindi TTS** | Which compliant path un-gates Hindi synthesis? | **Updated 2026-08-24 (M39): IMPLEMENTED on the local/staging tier.** Kokoro Hindi (Option A) serves hindi-female (hf_alpha) and hindi-male (hm_psi) through the existing M35-M37 architecture: clean RT-WER 0.0707/0.0547 (gate ≤0.08 PASS), truncation fixed, streaming + billing + playback reused, Web E2E verified. Remaining gates: founder listening (M38 audition pack, UNSCORED) and the PRODUCTION promotion (M24-style commit replacing the PENDING sentinel) + the TTS production launch itself ([M39 doc](../milestones/39-kokoro-hindi-tts-local-web.md)). |
